...
class DriverResult:
    ...
    # Aggregated across every gateway call this driver made — surfaced by
    # BrowserSkill so the per-node AgentResult carries real cost/provider
...
class BaseDriver:
    ...
    def _finalize(self, result: DriverResult) -> DriverResult:
        """Roll per-turn provider/cost/tokens up onto the DriverResult so
        BrowserSkill can attribute real numbers to the node's AgentResult."""
        result.cost = round(sum(s.cost for s in self.steps), 6)
        result.tokens_in = sum(s.tokens_in for s in self.steps)
        result.tokens_out = sum(s.tokens_out for s in self.steps)
        # Use the last non-empty provider seen — drivers normally stay on
        # one provider for the whole run, but failovers can swap mid-run.
        ...
        for turn in range(1, self.config.max_steps + 1):
            ...
            if "error" in last.outcome:
                ...
                if failures >= self.config.max_failures:
                    return self._finalize(DriverResult(
                        False, f"giveup after {failures} consecutive failures",
                        steps=self.steps))
            else:
                failures = 0
            if done:
                return self._finalize(DriverResult(success, note, steps=self.steps))
        return self._finalize(DriverResult(
            False, f"step cap reached ({self.config.max_steps})",
            steps=self.steps))
...
class SetOfMarksDriver(BaseDriver):
    ...
    async def _decide(self, snap: PageSnapshot, turn: int):
        ...
        if self.config.artifacts_dir:
            ...
            (d / f"turn_{turn:02d}_snapshot.json").write_text(_json.dumps({
                "dpr": snap.dpr,
                "viewport": {"w": snap.viewport_w, "h": snap.viewport_h},
                "elements": [
