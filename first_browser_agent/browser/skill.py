...
# Pages we know are interstitials (consent, cookie walls, region
# redirects). When the extract layer hits one, fall through to the
# Playwright layers — a real browser session can dismiss the banner or
# the SPA can post the consent cookie itself. Conservative list: only
# patterns that strongly imply "this is NOT the page you asked for".
_INTERSTITIAL_URL_FRAGMENTS = (
    "consent.google.",
    "consent.youtube.",
    "/sorry/index",            # google sorry / robot-check redirect
    "accounts.google.com/signin",
)
_INTERSTITIAL_CONTENT_NEEDLES = (
    "before you continue to google",
    "before you continue to youtube",
    "we use cookies and data to",   # google consent body
)


def _looks_like_interstitial(content: str, final_url: str) -> bool:
    u = (final_url or "").lower()
    if any(frag in u for frag in _INTERSTITIAL_URL_FRAGMENTS):
        return True
    c = (content or "").lower()[:2000]
    return any(n in c for n in _INTERSTITIAL_CONTENT_NEEDLES)


def _is_useful_extract(content: str, goal: str, final_url: str = "") -> bool:
    ...
    (< ~200 chars), known interstitial pages (consent / cookie walls /
    sign-in redirects), or the case where the page rendered but the goal
    asks for an interaction (`click`, `fill`, `select`, etc.) — for those
    goals extraction is never sufficient regardless of content length."""
    if len(content) < 200:
        return False
    if _looks_like_interstitial(content, final_url):
        return False
...
class BrowserSkill:
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        # Prefer a stable node-id tagged dir when the caller supplies one
        # (replace ':' with '_' so the path is portable). Fall back to a
        # timestamp directory for legacy callers.
        _nid = node.metadata.get("node_id") if isinstance(node.metadata, dict) else None
        if self.artifacts_root and _nid:
            artifacts_dir = str(self.artifacts_root / f"browser_{str(_nid).replace(':', '_')}")
        elif self.artifacts_root:
            artifacts_dir = str(self.artifacts_root / f"browser_{int(t0)}")
        else:
            artifacts_dir = None
        ...
        #
        # `force_path` from the caller overrides the natural cascade. When
        # it pins a Playwright layer ("a11y" or "vision"), we must skip
        # Layer 1's static-extract short-circuit — otherwise a thin static
        # HTML body (e.g. Google's JS shell) would be accepted as the
        # answer and the requested layer never runs. We still keep the
        # gateway-block sniff so we fail fast on obvious CAPTCHAs.
        ...
        skip_layer1_extract = force_path in {"a11y", "vision"}
        if html:
            ...
            if not skip_layer1_extract:
                content = _extract(html)
                if _is_useful_extract(content, goal, final_url):
    ...
    def _pack_driver(self, path, url, goal, drv_result,
        ...
        return AgentResult(
            success=True, agent_name=self.NAME,
            output=out.model_dump(), elapsed_s=elapsed,
            provider=getattr(drv_result, "provider", "") or "",
            cost=float(getattr(drv_result, "cost", 0.0) or 0.0),
        )
