...
def render_prompt(skill: Skill, query: str, resolved: list[dict],
    ...
    # TODAY: real wall-clock date. Without this the model defaults to its
    # training-cutoff intuition ("June 2026 isn't yet bookable") and refuses
    # to act on future-dated queries that are actually in the present /
    # near future. Stamping the real date on every prompt fixes the bias
    # at the source so Planner / Browser / Distiller / Critic / Formatter
    # all reason against the real calendar.
    import datetime as _dt
    parts += ["", f"TODAY: {_dt.date.today().isoformat()}"]
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "browser":
        ...
        sk = BrowserSkill(
            ...
            session=session_id,
            # Heavy interactive SPAs (Google Flights, OTAs) routinely need
            # 8-14 vision turns just to dismiss consent + fill origin / dest
            # / depart / return + sort + read three cards. The default 12
            # was tight enough that the cascade hit the step cap before
            # reaching the results list. Bump headroom and the wall clock
            # to match.
            max_steps_vision=24,
            max_steps_a11y=14,
            wall_clock_s=240.0,
