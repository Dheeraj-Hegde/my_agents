...
def _safe(path: str) -> Path:
    # All file tools are already rooted at SANDBOX. Strip a redundant leading
    # "sandbox/" (or "./sandbox/") that callers sometimes include in the path,
    # which would otherwise resolve to a non-existent sandbox/sandbox/... dir.
    cleaned = path.replace("\\", "/").lstrip("/")
    while cleaned.startswith("./"):
        cleaned = cleaned[2:]
    if cleaned.lower().startswith("sandbox/"):
        cleaned = cleaned[len("sandbox/"):]
    p = (SANDBOX / cleaned).resolve()
