...
# Locate llm_gatewayV9. The historical layout was sibling-of-grandparent
# (`parents[2] / "llm_gatewayV9"`), but in this workspace the gateway
# lives at `first_computer_agent / llm_gatewayV9` (a child of the agent
# code, not a sibling). Try the override env var first, then the in-tree
# child path, then the legacy sibling path — first one that exists wins.
def _resolve_gateway_dir() -> Path:
    env = os.environ.get("GATEWAY_V9_DIR")
    if env:
        return Path(env).expanduser().resolve()
    here = Path(__file__).resolve()
    candidates = [
        here.parent / "llm_gatewayV9",          # in-tree child (this repo)
        here.parents[2] / "llm_gatewayV9",      # legacy sibling-of-grandparent
        here.parents[1] / "llm_gatewayV9",      # one level up
    ]
    for c in candidates:
        if c.exists():
            return c
    return candidates[0]  # report the first miss so the error is actionable


GATEWAY_V9_DIR = _resolve_gateway_dir()
