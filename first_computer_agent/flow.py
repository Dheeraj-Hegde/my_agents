...
def main() -> None:
    ...
    # Dashboard hand-off: when the dashboard spawns flow.py it sets
    # S8_SESSION_ID so the trajectory directory it polls
    # (state/trajectories/<sid>/) matches the sid it advertised to
    # the UI. This is NOT a --resume — the sid is fresh, we just want
    # the orchestrator to honour it instead of minting its own.
    import os as _os
    env_sid = _os.environ.get("S8_SESSION_ID")
    if env_sid and not explicit_resume and resume_sid is None:
        resume_sid = env_sid
