...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer_use":
        # Same shape as `browser`: the Computer-Use skill owns its own
        # five-layer cascade (api → hotkeys → uia → electron → vision)
        # and writes a trajectory directory under
        # state/trajectories/<session>/<task>/. We dispatch by importing
        # the task module the node names in metadata.task_module.
        import importlib

        node_dict = graph_nodes[node_id]
        meta = node_dict.get("metadata") or {}
        task_module = meta.get("task_module")
        if not task_module:
            return AgentResult(
                success=False, agent_name=skill.name,
                error="computer_use node missing metadata.task_module",
                elapsed_s=time.time() - started,
            ), ""

        # Forward optional task-specific kwargs through `metadata.task_args`
        # so the Planner can parameterise a task (e.g. operands on the
        # calculator) without us touching the task module per query.
        # Kept under a dedicated key (rather than splatting all of `meta`)
        # so orchestrator-reserved keys like task_module / enabled_layers
        # / label / question never collide with a task's build() params.
        task_args = meta.get("task_args") or {}
        if not isinstance(task_args, dict):
            return AgentResult(
                success=False, agent_name=skill.name,
                error=f"metadata.task_args must be a dict, got {type(task_args).__name__}",
                elapsed_s=time.time() - started,
            ), ""

        try:
            mod = importlib.import_module(task_module)
            task_spec = mod.build(**task_args)
        except Exception as exc:
            return AgentResult(
                success=False, agent_name=skill.name,
                error=f"could not load task_module {task_module!r}: {exc}",
                elapsed_s=time.time() - started,
            ), ""
        ...
        sk = ComputerUseSkill(
            session=session_id,
            enabled_layers=meta.get("enabled_layers"),
        )
        # Run the cascade off the event loop so its blocking pyautogui /
        # uiautomation / playwright-sync calls do not stall asyncio.
        cu_out = await asyncio.to_thread(sk.run, task_spec)
        return AgentResult(
            success=cu_out.success,
            agent_name=skill.name,
            output=cu_out.model_dump(),
            elapsed_s=time.time() - started,
        ), ""
