...
structured records the Formatter can render cleanly.
  ...
  computer_use       drive the host OS through a five-layer cascade
                     (api → hotkeys → uia → electron → vision). Use
                     when the user asks to OPEN / OPERATE a desktop
                     app (Calculator, Notepad, VS Code, Paint, etc.)
                     or to demonstrate a real on-screen action — NOT
                     for problems that can be answered by text alone.
                     metadata MUST set:
                       - task_module (str, dotted path to a
                         `computer_use.tasks.*` module exposing
                         `build(**kwargs) -> TaskSpec`). Known
                         modules:
                           computer_use.tasks.calculator      —
                             Windows Calculator. Accepts
                             task_args = {"a": int, "b": int,
                             "op": "+"|"-"|"*"|"/"}.
                           computer_use.tasks.vscode_editor   —
                             VS Code via Electron CDP. Reports the
                             list of open editor tabs.
                           computer_use.tasks.vscode_create_file —
                             Drives VS Code to author a new file.
                             Accepts task_args = {"file_path": str
                             (defaults to "readme_demo.txt"),
                             "content": str (defaults to a demo
                             sentence)}. Use this whenever the user
                             asks to OPEN VS Code AND CREATE / WRITE
                             a small text file (and optionally close
                             VS Code afterwards).
                           computer_use.tasks.canvas_sketch   —
                             MS Paint via Layer-3 vision.
                           computer_use.tasks.browser_game    —
                             Plays a bundled in-browser
                             click-the-red-target mini-game using
                             Layer-3 vision only (no DOM, no a11y,
                             no hotkeys). Use this whenever the
                             user asks the agent to PLAY a BROWSER
                             GAME, or to demonstrate vision-only
                             control / clicks in a web page.
                             Takes no task_args.
                       - task_args (dict, optional) forwarded to the
                         task's `build()` as **kwargs. Use this to
                         parameterise a task (e.g. operands for the
                         calculator) without editing the task module.
                     Example: user asks "open the Windows Calculator
                     and compute 15350 + 2150" →
                       {"skill":"computer_use","inputs":[],
                        "metadata":{"label":"calc",
                                    "task_module":"computer_use.tasks.calculator",
                                    "task_args":{"a":15350,"b":2150,"op":"+"}}}
