...
SYSTEM = (
    ...
    "   start with `[sandbox:` or `[artifact:` (these mark previously-indexed\n"
    ...
    "   If MEMORY HITS already contain a `fact` or `preference` item whose\n"
    "   descriptor visibly answers the user's question (e.g. the user asks\n"
    "   'when is mom's birthday?' and a fact descriptor says\n"
    "   \"mom's birthday is 15 May 2026\"), DO NOT collapse to a single\n"
    "   answer goal. Emit TWO goals: first a locate-from-memory goal\n"
    "   (\"Identify mom's birthday from the existing memory hits\") and\n"
    "   then a final user-facing answer goal (\"Answer the user's\n"
    "   question about mom's birthday\"). Both will be satisfied by\n"
    "   Decision producing an ANSWER on successive iterations — no tool\n"
    "   call is needed. Never collapse to one goal just because the\n"
    ...
    "Example. Given\n"
    '  MEMORY HITS: [{"i":0,"artifact_id":7,"descriptor":'
    '"page fetch result -> artifact:7"}]\n'
