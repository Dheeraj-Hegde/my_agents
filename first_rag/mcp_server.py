...
# Windows console defaults to cp1252, so crawl4ai/Rich logging Unicode chars
# (e.g. '\u2192' arrows) explodes with a 'charmap' codec error mid-tool-call.
# Force UTF-8 with replacement for both streams before anything else writes.
...
def _read_for_index(path: str) -> tuple[str, str]:
    ...
    Artifact handles are passed in textual form `artifact:<int>` (see the
    artifact-handle convention in schemas.py and decision.py)."""
    if path.startswith("artifact:"):
        try:
            art_id = int(path.removeprefix("artifact:"))
        except ValueError as e:
            raise ValueError(f"malformed artifact handle: {path!r}") from e
        return _artifacts.get_bytes(art_id).decode("utf-8", errors="replace"), f"artifact:{art_id}"
    p = _safe(path)
...
def index_document(path: str, chunk_size: int = 400, overlap: int = 80) -> dict:
    ...
    # Batch-embed all chunks of this file in a single gateway round-trip.
    # Per-chunk add_fact() was paying one HTTP call + provider cooldown
    # per chunk, turning a 25-chunk file into a multi-minute serial loop.
    facts = []
    ...
    _memory.add_facts_batch(facts, source=source, run_id=run_id)
    return {
        "path": path,
        "source": source,
...
def search_knowledge(query: str, k: int = 5) -> list[dict]:
    ...
    # Cosine threshold below which a chunk is treated as topically unrelated
    # noise rather than a real hit. nomic-embed-text typically scores
    # genuinely-on-topic chunks > 0.55 and tangential chunks 0.3-0.5; the
    # default 0.5 is conservative. Tune via SEARCH_MIN_SCORE if the corpus
    # or embedder changes. The threshold matters because without it the
    # FAISS top-k always returns k chunks, and the agent will confabulate
    # an answer from whatever was closest — even for queries the corpus
    # has no information about (e.g. "Is flutter planning any layoffs?"
    # over an earnings-call corpus that never mentions layoffs).
    try:
        min_score = float(os.getenv("SEARCH_MIN_SCORE", "0.5"))
    except ValueError:
        min_score = 0.5

    scored = _memory.vector_search_scored(
        query, kinds=["fact"], top_k=max(k * 4, 20), min_score=min_score
    )
    chunk_items = [
        (it, score) for (it, score) in scored
        if isinstance(it.value, dict) and isinstance(it.value.get("chunk"), str)
        and it.value["chunk"].strip()
    ][:k]
    return [
        {
            "id": item.id,
