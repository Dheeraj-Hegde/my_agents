...
def embed_many(texts: list[str], task_type: str = "retrieval_document") -> list[list[float]]:
    """Batched embedding via the gateway's `/v1/embed_batch` endpoint.

    One HTTP round-trip, one provider rate slot, many vectors. Use this
    for bulk indexing instead of looping `embed()` — Gemini's free tier
    counts batch calls as a single quota slot, so batching is the lever
    that turns minutes of cooldown waits into a single request.
    Returns the raw vectors in input order."""
    ensure_gateway()
    if LLM is None:
        raise RuntimeError(
            "Gateway V7 client unavailable. Confirm llm_gatewayV7/client.py exists."
    ...
    if not texts:
        return []
    resp = LLM().embed_batch(texts, task_type=task_type)
    return resp["embeddings"]


__all__ = ["ensure_gateway", "LLM", "GATEWAY_URL", "GATEWAY_V7_DIR", "embed", "embed_many"]
