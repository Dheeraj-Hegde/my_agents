...
from gateway import LLM, embed as _gateway_embed, embed_many as _gateway_embed_many, ensure_gateway
...
def vector_search_scored(
    ...
    *,
    ...
    """Public variant that returns `(item, cosine_similarity)` pairs so
    callers (e.g. `search_knowledge`) can apply a relevance threshold and
    expose the score to the agent. Cosine because the FAISS index is
    ...
    if idx.size == 0:
        return []
    hits = idx.search(qvec, k=top_k * 4 if kinds else top_k * 2)
    if not hits:
        return []
    by_id: dict[str, MemoryItem] = {item.id: item for item in _load()}
...
def _fallback_remember(
    ...
    # See note in remember(): user_query items skip the vector index.
    embedding = (
        None if source == "user_query"
        else _try_embed(descriptor, task_type="retrieval_document")
    )
    item = MemoryItem(
        id=new_id("mem"),
        kind="fact",
...
def remember(
    ...
    *,
    ...
    # `user_query` items (the agent records every incoming user question via
    # memory.remember at run start so any facts inside it survive future runs)
    # must NOT be vector-indexed: their descriptor is derived from the question
    # itself, so they score cosine~1 against that same question on the very
    # next memory.read() and crowd out every real indexed document chunk. They
    # remain reachable via keyword search and through future remember() calls.
    embedding: list[float] | None = None
    if c.kind in _EMBEDDABLE_KINDS and source != "user_query":
...
def add_fact(
    ...
    *,
    ...
    classifier (kind is known) but still embeds. When `embed_text` is
    provided it is used as the vector input (so callers can keep the
    short descriptor as a label while embedding the full body); otherwise
    the descriptor itself is embedded."""
    embedding = _try_embed(embed_text or descriptor, task_type="retrieval_document")
    item = MemoryItem(
        id=new_id("mem"),
        kind="fact",
...
def add_facts_batch(
    ...
    *,
    ...
    """Batched variant of `add_fact` for bulk indexing. Each entry in
    `facts` is a dict with keys: descriptor (required), embed_text
    (optional, defaults to descriptor), value (optional), keywords
    (optional). All embeddings are computed in a single gateway round-trip
    via `embed_many`, then the items are persisted to memory.json and the
    FAISS index in one pass. Falls back per-item to `_try_embed` if the
    batch call fails, so callers always get persisted items even when
    the gateway is degraded."""
    if not facts:
        return []

    embed_inputs = [(f.get("embed_text") or f["descriptor"]) for f in facts]
    try:
        ensure_gateway()
        vectors: list[list[float] | None] = list(
            _gateway_embed_many(embed_inputs, task_type="retrieval_document")
        )
        if len(vectors) != len(facts):
            raise RuntimeError(
                f"embed_many returned {len(vectors)} vectors for {len(facts)} inputs"
            )
    except Exception as e:
        print(f"[memory.add_facts_batch] batch embed failed ({e!r}); falling back to per-item embed")
        vectors = [_try_embed(t, task_type="retrieval_document") for t in embed_inputs]

    items = _load()
    idx = _index()
    appended: list[MemoryItem] = []
    for f, vec in zip(facts, vectors):
        descriptor = f["descriptor"]
        keywords = f.get("keywords") or list(_tokens(descriptor))[:10]
        item = MemoryItem(
            id=new_id("mem"),
            kind="fact",
