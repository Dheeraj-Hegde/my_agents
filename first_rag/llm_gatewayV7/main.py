...
# Windows console defaults to cp1252; logged payloads with non-Latin chars
# (e.g. '\u2192') crash uvicorn's access/error logging. Force UTF-8.
...
from schemas import (
    ChatRequest, ChatResponse, ToolCall, RouterDecision,
    EmbedRequest, EmbedResponse, EmbedBatchRequest, EmbedBatchResponse,
)
import embedders as E

DEFAULT_ORDER = ["ollama",  "cerebras", "gemini", "nvidia", "groq", "openrouter", "github"]
...
PORT = int(os.getenv("GATEWAY_V7_PORT", "8101"))
...
async def embed_batch(req: EmbedBatchRequest):
    """Batched embed: one HTTP call, one rate slot, many vectors. Use this
    for bulk document indexing — gemini's quota is per-call, so batching
    25 chunks into one request turns a 2-minute serial loop into one call."""
    embedders = app.state.embedders
    if not embedders:
        raise HTTPException(503, "no embedding providers configured")
    if not req.texts:
    ...
    for i, t in enumerate(req.texts):
        if len(t) > E.MAX_INPUT_CHARS:
            raise HTTPException(
                413,
                f"texts[{i}] is {len(t)} chars; embed input is capped at "
                f"{E.MAX_INPUT_CHARS} chars. Chunk the input before embedding.",
            )

    total_chars = sum(len(t) for t in req.texts)
    t0 = time.time()
    try:
        name, result, attempts, latency = await E.embed_batch_with_failover(
            embedders, req.texts, req.task_type, explicit=req.provider
        )
    except E.EmbedderError as e:
        latency = int((time.time() - t0) * 1000)
        db.log_call(
        ...
        if req.provider:
            ...
            if e.status == 400:
                raise HTTPException(400, str(e))
            raise HTTPException(502, f"{req.provider} embed_batch failed: {e}")
        raise HTTPException(503, str(e))
