...
class EmbeddingProvider:
    ...
    async def embed_batch(self, texts: list[str], task_type: TaskType) -> dict:
        """Embed multiple inputs in a single provider call when supported.
        Default impl is a sequential loop; subclasses can override with a
        true batch API (e.g. gemini batchEmbedContents). Returns
        {"embeddings": list[list[float]], "model": str, "dim": int}."""
        vecs: list[list[float]] = []
        model = self.model
        dim = 0
        for t in texts:
            r = await self.embed(t, task_type)
            vecs.append(r["embedding"])
            model = r["model"]
            dim = r["dim"]
        return {"embeddings": vecs, "model": model, "dim": dim}
...
class GeminiEmbedder(EmbeddingProvider):
    ...
    async def embed_batch(self, texts: list[str], task_type: TaskType) -> dict:
        """Use gemini's batchEmbedContents endpoint — one HTTP call, one rate
        slot, for up to 100 inputs. The free tier accepts the same per-minute
        bucket as singular calls, so batching is the real lever for throughput."""
        if not texts:
            return {"embeddings": [], "model": self.model, "dim": self.output_dim}
        url = (
            f"https://generativelanguage.googleapis.com/v1beta/"
            f"models/{self.model}:batchEmbedContents?key={self.api_key}"
        )
        task = self._TASK_MAP[task_type]
        body = {
            "requests": [
                {
                    "model": f"models/{self.model}",
        ...
        if r.status_code != 200:
            raise EmbedderError(
                f"gemini batch HTTP {r.status_code}: {r.text[:200]}", status=r.status_code
            )
        d = r.json()
        raw = d.get("embeddings") or []
        vecs = [(e.get("values") or []) for e in raw]
        if len(vecs) != len(texts) or any(not v for v in vecs):
            raise EmbedderError(
                f"gemini batch returned {len(vecs)} vectors for {len(texts)} inputs: {str(d)[:200]}"
            )
        return {"embeddings": vecs, "model": self.model, "dim": len(vecs[0])}
...
async def embed_batch_with_failover(
    ...
    """Batch variant of embed_with_failover. Same can_use/record/mark_failure
    semantics, but the entire batch counts as a single rate slot per provider —
    that's the whole point of batching against quota-limited embedders.
    Returns (name, {"embeddings": list[list[float]], "model": str, "dim": int},
    attempts, latency_ms)."""
    attempts: list[dict] = []
    candidates = embedders
    if explicit:
