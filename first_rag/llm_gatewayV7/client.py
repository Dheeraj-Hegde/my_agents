...
DEFAULT_URL = os.getenv("LLM_GATEWAY_V7_URL", "http://localhost:8101")
...
class LLM:
    ...
    def embed_batch(self, texts: list[str],
                    task_type: str = "retrieval_document",
                    provider: Optional[str] = None) -> dict:
        """Returns {provider, model, embeddings: list[list[float]], dim,
        latency_ms, attempted}. One HTTP call, one provider rate slot,
        many vectors — preferred over looping `embed()` for indexing."""
        body = {"texts": texts, "task_type": task_type}
        if provider:
            body["provider"] = provider
        r = httpx.post(f"{self.base_url}/v1/embed_batch", json=body, timeout=self.timeout)
        r.raise_for_status()
        return r.json()
