...
def build_providers(cache_store):
    ...
    if k := os.getenv("GEMINI_API_KEY"):
        out["gemini"] = GeminiProvider(k, os.getenv("GEMINI_MODEL", "gemma-4-31b-it"), cache_store)
    if k := os.getenv("NVIDIA_API_KEY"):
        out["nvidia"] = NvidiaProvider(k, os.getenv("NVIDIA_MODEL", "deepseek-ai/deepseek-v4-flash"))
